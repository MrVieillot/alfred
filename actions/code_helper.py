...
def _write(description: str, language: str, output_path: str, player=None) -> tuple[str, Path]:
    ...
    system = (
        ...
        "Return ONLY code. No markdown, no explanation, no backticks."
    )

    prompt = f"""Write clean, working, well-commented {lang} code for the description below.
...
Code:"""
    ...
    code = _clean_code(_ask_local(prompt, system=system))
...
def _fix_code(code: str, error_output: str, description: str) -> str:
    system = (
        ...
        "Return ONLY corrected code. No markdown, no explanation, no backticks."
    )
...
Fixed code:"""
    ...
    return _clean_code(_ask_local(prompt, system=system))
...
def _edit_action(file_path, instruction, player) -> str:
    ...
    try:
        edited = _clean_code(_ask_local(prompt, system=system))
    except Exception as e:
...
Optimized code:"""
    ...
    try:
        optimized = _clean_code(_ask_local(
            prompt,
            system=f"You are an expert {lang} developer. Return ONLY optimized code."
        ))
    except Exception as e:
...
def _screen_debug_action(description, file_path, player, speak=None) -> str:
    return (
        "Screen debug is not available in full local mode yet. "
        "We need to migrate this to a local vision model such as llava or qwen-vl."
    )
