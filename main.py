...
class JarvisLocal:
    ...
    def _build_system_prompt(self) -> str:
        ...
        parts.append(
            "\n[LOCAL TOOL MODE]\n"
            "You can either answer normally or request a tool call.\n"
            "If a tool is needed, return ONLY valid JSON like this:\n"
            '{"type":"tool","name":"open_app","args":{"app_name":"Chrome"}}\n'
            "If no tool is needed, return ONLY valid JSON like this:\n"
    ...
    def _handle_command_sync(self, text: str):
        ...
        if lowered.startswith(("search ", "search the web for ", "look up ", "google ")):
            ...
            for prefix in ("search the web for ", "search ", "look up ", "google "):
            ...
            if name:
                ...
                followup = ask_ollama(
                    prompt=(
                        ...
                        "Reply to the user in one short natural sentence."
                    ),
                    system="You are JARVIS. Be concise, direct, and address the user as sir.",
                    model="qwen3.5:4b"
                )

                self.speak(followup.strip())

            else:
                answer = data.get("content", raw)
                self.speak(str(answer).strip())

        except Exception as e:
            traceback.print_exc()
            self.speak_error("local_llm", e)
    ...
    def _execute_tool_sync(self, name: str, args: dict) -> str:
        if name == "browser_control":
            ...
            browser = args.get("browser", "chrome").lower()
            url = args.get("url", "")
        ...
        if name == "browser_control":
            ...
            if args.get("action") in action_map:
                args["action"] = action_map[args["action"]]

            if args.get("action") == "go_to" and "url" not in args:
...
def main():
    ...
    def runner():
        ...
        ui.write_log("SYS: JARVIS local mode online.")
